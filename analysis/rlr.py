...
class RandomizedRegression:
    def __init__(self, is_continuous=False, model_dir=None, log_l1_range=False, count_absolute_threshold=None):
        ...
        self.count_absolute_threshold = count_absolute_threshold
        self.feature_counts = {}
    
    def get_salient_features(self, feature_key, target_key, n=100):
        if not self.feature_counts:
            print('Need to fit model first')
            return None
        else:
            rev_cnts = [[(v, feature_key[k]) for k, v in self.feature_counts[i].items()]
    ...
    def fit_batch(self, X, y, l1, batch_idx):
        ...
        # count which features are positive, or have large absolute value
        if len(wts.shape) == 1:
            if 0 not in self.feature_counts:
                self.feature_counts[0] = {}
            
            for i, v in enumerate(wts):
                if (self.count_absolute_threshold and np.abs(v) > self.count_absolute_threshold) or (not self.count_absolute_threshold and (v > 0)):
                    if i not in self.feature_counts[0]:
                        self.feature_counts[0][i] = 0
                    self.feature_counts[0][i] += 1
        else:
            for target in range(wts.shape[0]):
                if target not in self.feature_counts:
                    self.feature_counts[target] = {}
                
                for i, v in enumerate(wts[target]):
                    if (self.count_absolute_threshold and np.abs(v) > self.count_absolute_threshold) or (not self.count_absolute_threshold and (v > 0)):
                        if i not in self.feature_counts[target]:
                            self.feature_counts[target][i] = 0
                        self.feature_counts[target][i] += 1
