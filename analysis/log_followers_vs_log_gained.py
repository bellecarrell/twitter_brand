...
def main(in_dir, out_dir):
    ...
    for u in users:
        ...
        if not math.isinf(pc) and not math.isnan(pc):
            ...
            x.append(np.log(fc))
            y.append(np.log(followers_gained))

    plt.scatter(x,y)
    plt.xlabel('Log Followers Count')
    plt.ylabel('Log Followers Gained')
    #plt.show()
    fname = os.path.join(out_dir,'log_followers_vs_followers_gained.png')
    plt.savefig(fname)

if __name__ == '__main__':
    """
    Create plot of log followers vs log gained.
    """
