...
def fit_nmf(train_max, heldout_max=None, vocab=None, k=10, alpha_regularization=0.0):
    ...
    for iteration in range(20):
        train_idxes = np.random.choice(train_nmf.shape[0], batch_size, replace=True)
        ...
        if heldout_nmf is not None:
            heldout_idxes = np.random.choice(heldout_nmf.shape[0], batch_size, replace=True)
...
def fit_lda(train_max, heldout_max=None, vocab=None, k=10, alpha=1.0, beta=10**-3):
    lda = LatentDirichletAllocation(k, alpha, beta, learning_method='batch', verbose=True)
