'''
Fit model with all hypotheses and controls and then
'''
...
# outcomes to predict
REGRESSION_DEP_VARS_FMT = ['pct_follower_change_{}-horizon']
...
def main(train_path, dev_path, test_path, horizon, model_class, out_dir, args_obj):
    ...
    # mean-normalize follower count % change in each fold
    for v in dep_vars:
        tr_df[v] = tr_df[v] - np.nanmean(tr_df[v])
        dev_df[v] = dev_df[v] - np.nanmean(dev_df[v])
        tst_df[v] = tst_df[v] - np.nanmean(tst_df[v])
