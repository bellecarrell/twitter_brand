...
def fit_batches(in_dir, static_info,  model_dir, batch='month', l1_range=[0.0, 1.0]):
    np.random.seed(SEED)
...
def main(in_dir,out_dir):
    ...
    for tw in batch_tws:
        ...
        low, high = fit_batches(in_dir, static_info,model_dir, l1_range=[0.0, 10.0])
        ...
        ensemble.eval_to_file(X,y_l, os.path.join(out_dir,'eval_{}_batch_{}'.format(name,tw)))

        high_md = model_dir + '_high'
        ensemble = Ensemble(high_md)
        ensemble.eval_to_file(X,y_h, os.path.join(out_dir,'eval_{}_batch_{}'.format(name,tw)))
