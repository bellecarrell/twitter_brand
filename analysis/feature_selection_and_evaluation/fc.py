...
def main(in_dir,out_dir):
    ...
    batch_tws = ['month', 'all_data']

    for tw in batch_tws:
        model_dir = os.path.join(out_dir, '/models/lr_log_reg_models_{}_{}'.format(name,tw))

        low, high = fit_batches(in_dir, static_info,model_dir, tw, l1_range=[0.0, 10.0])
        ...
        X, us = load_fold(in_dir, vocab, static_info,'test')
        X, y_l, y_h = filter_by_stratification(X, us, static_info, len(vocab))

        #train_lr_with_salient_features(in_dir,out_dir,vocab,static_info,low_sf,filter_by_stratification,1)
        ...
        low, high = fit_batches(in_dir, static_info, model_dir, tw, l1_range=[0.0, 10.0], model_type='svm')
        low_sf = low.get_salient_features(dict((v, k) for k, v in vocab.items()), {0: 'low'}, n=100)
        high_sf = high.get_salient_features(dict((v, k) for k, v in vocab.items()), {0: 'high'}, n=100)
        salient_features = dict(low_sf, **high_sf)
        ...
        train_lr_with_salient_features(in_dir, out_dir, vocab, static_info, sf, filter_by_stratification, 1, tw)
