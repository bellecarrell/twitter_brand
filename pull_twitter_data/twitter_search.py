...
import pull_twitter_data.twitterUtils as twitterUtils
    ...
    def getPastTweets(self):
        ...
        for user in totalUsers:
            ...
            while (userTimeline or tryAgain) and len(userAllStatuses) < NUM_TO_CACHE:
                try:
                    print('Searching for statuses from', user, 'of max ID', maxId, 'current count',
                          len(userAllStatuses))
                    tryAgain = False
                    if maxId > 0:
                        try:
                            ...
                            userTimeline = api.user_timeline(screen_name=user, max_id=maxId, count=200,
                                                             include_rts=True)
                    else:
                        try:
                            u = int(user)
                    ...
                    try:
                        ...
                        print('Error code %d: Rate limit hit, snoozing for %s seconds' % (
                        e2.response, str(limit['reset_time_in_seconds'])))
